...
class InteractionMirror:
    ...
    def apply_full(self, room_id, zone_id, interactions):
    ...
    def apply_start(self, room_id, zone_id, object_key, player_id, interaction, started_at, args=None, affordance=None, affordance_id=None, target=None):
        if room_id != self.room_id or (zone_id is not None and zone_id != self.zone_id):
            return
    ...
    def apply_free(self, room_id, zone_id, object_key, cooldown_until):
        if room_id != self.room_id or (zone_id is not None and zone_id != self.zone_id):
            return
